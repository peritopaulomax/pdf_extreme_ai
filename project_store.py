...
class ProjectStore:
    ...
    def list_projects(self) -> list[ProjectRecord]:
        data = self._load()
        return [ProjectRecord(**p) for p in data["projects"]]
    ...
    def create_project(self, name: str) -> ProjectRecord:
        ...
        lexical_db = f".lexical_{project_id}.db"
        checkpoint = f".ingest_checkpoint_{project_id}.json"
        uploads_dir = f"projects_data/{project_id}/uploads"
    ...
    def set_global_rules(self, project_id: str, rules: str) -> ProjectRecord:
        ...
        project.global_rules = (rules or "").strip()
...
def project_uploads_dir(project_id: str) -> Path:
    path = Path("projects_data") / project_id / "uploads"
